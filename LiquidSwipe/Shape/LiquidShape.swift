...
//   var side: Side
   ...
   func path(in rect: CGRect) -> Path {
      ...
      //Строить "кляксу" по 4 точкам Безье
      let A = CGPoint(x: 0, y: location.y + 80 + offset.width)
      let B = CGPoint(x: 0 + offset.width, y: location.y)
      let C = CGPoint(x: 0, y: location.y - 80 - offset.width)
      
      let ABcontrol1 = CGPoint(x: 0, y: (A.y + B.y) / 2)
      let ABcontrol2 = CGPoint(x: 0 + offset.width, y: (A.y + B.y) / 2)
      let BCcontrol1 = CGPoint(x: 0 + offset.width, y: (B.y + C.y) / 2)
      let BCcontrol2 = CGPoint(x: 0, y: (B.y + C.y) / 2)
